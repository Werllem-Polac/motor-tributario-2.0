...
def gerar_dashboard(dados):
    if dados is None:
        print("[ERRO] Nenhum dado foi carregado para gerar o Dashboard.")
        return
    ...
    # 1️⃣ Gráfico: Estratégias aplicadas por tipo de produto
    estrategias_produto = []
    ...
    # 2️⃣ Gráfico: Economia simulada por tipo de estratégia
    economia_estrategias = []

    for index, nota in dados.iterrows():
        valor = float(nota['Valor_Produto'])
        produto = nota['Produto']
        ncm = str(nota['NCM'])
    ...
    # 3️⃣ Gráfico: Comparativo de tributos (Antes x Depois)
    total_valor_produtos = dados['Valor_Produto'].sum()
    ...
    # Simulação de estratégias aplicadas
    ...
    # 4️⃣ Gráfico: Top 10 Produtos que mais economizam
    top10_economia = df_economia.copy()
    top10_economia['Produto'] = dados['Produto']
    ...
    # 5️⃣ Gráfico: Estratégias que mais impactam na economia
    plt.figure(figsize=(10,6))
    ...
    # 6️⃣ Gráfico: Produtos sem estratégia clara (alerta)
    ...
    # 7️⃣ Gráfico: CFOP Interno vs Interestadual
    ...
    print("\n✅ Gráficos avançados gerados e salvos na pasta 'graficos/'.")